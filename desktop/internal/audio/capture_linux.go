...
// NewRecorder creates a new Linux ALSA recorder.
func NewRecorder() (Recorder, error) {
	...
	// Configure hardware parameters: 16kHz, mono, S16_LE.
	...
	if ret = alsaSetAccess(handle, hwparams); ret < 0 {
		C.snd_pcm_close(handle)
	...
	bufferSize := C.snd_pcm_uframes_t(1600) // 100ms buffer
...
func alsaSetAccess(handle *C.snd_pcm_t, hwparams *C.snd_pcm_hw_params_t) C.int {
	return C.snd_pcm_hw_params_set_access(handle, hwparams, C.SND_PCM_ACCESS_RW_INTERLEAVED)
}
...
func (r *alsaRecorder) Start() (<-chan []float32, error) {
	...
	// Re-open channels if previously stopped.
	select {
	case <-r.done:
